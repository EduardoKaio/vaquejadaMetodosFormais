...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(Gerenciador_Senhas),processar_resultado_disputa)==(size(fila_disputa_atual)>0 & jj: JULGAMENTO | @ss.(ss = fila_disputa_atual(1) ==> (jj = zero_boi ==> estado_senha,fila_disputa_atual,eliminados_nesta_rodada:=estado_senha<+{ss|->eliminada},fila_disputa_atual\|/1,eliminados_nesta_rodada<-ss [] not(jj = zero_boi) ==> (jj = valeu_boi ==> estado_senha,fila_disputa_atual,fila_disputa_proxima:=estado_senha<+{ss|->classificada},fila_disputa_atual\|/1,fila_disputa_proxima<-ss [] not(jj = valeu_boi) ==> estado_senha,fila_disputa_atual:=estado_senha<+{ss|->classificada},fila_disputa_atual\|/1<-ss))));
...
THEORY ListANYVarX IS
  ...
  List_ANY_Var(Machine(Gerenciador_Senhas),processar_resultado_disputa)==(Var(ss) == btype(INTEGER,?,?));
