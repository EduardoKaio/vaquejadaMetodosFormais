...
THEORY ListVariablesX IS
  ...
  Local_List_Variables(Machine(Controle_Torneio))==(posicao_atual,colocacao,premiacao_total_atual,premiacao_vagas,premiacao_individual,racha_sem_finalizar,ultimo_rodizio_iniciado,torneio_iniciado,idx_correndo,rodizio_atual,fase_atual);
  List_Variables(Machine(Controle_Torneio))==(posicao_atual,colocacao,premiacao_total_atual,premiacao_vagas,premiacao_individual,racha_sem_finalizar,ultimo_rodizio_iniciado,torneio_iniciado,idx_correndo,rodizio_atual,fase_atual,tamanho_buffer_saida,buffer_saida,eliminados_nesta_rodada,fila_disputa_proxima,fila_disputa_atual,fila_geral,bois_corridos,estado_senha,dono_senha);
  External_List_Variables(Machine(Controle_Torneio))==(posicao_atual,colocacao,premiacao_total_atual,premiacao_vagas,premiacao_individual,racha_sem_finalizar,ultimo_rodizio_iniciado,torneio_iniciado,idx_correndo,rodizio_atual,fase_atual,tamanho_buffer_saida,buffer_saida,eliminados_nesta_rodada,fila_disputa_proxima,fila_disputa_atual,fila_geral,bois_corridos,estado_senha,dono_senha)
END
&
...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(Controle_Torneio))==(fase_atual: FASES & rodizio_atual: iseq(NAT1) & size(rodizio_atual)<=TAMANHO_RODIZIO & idx_correndo: 1..TAMANHO_RODIZIO & torneio_iniciado: BOOL & ultimo_rodizio_iniciado: BOOL & racha_sem_finalizar: BOOL & premiacao_individual: VAQUEIROS --> NAT & premiacao_vagas: NAT & premiacao_total_atual: NAT & colocacao: SENHAS +-> NAT1 & posicao_atual: NAT & !ss.(ss: dom(estado_senha) & estado_senha(ss) = campea => fase_atual = encerrado & ss: dom(bois_corridos) & bois_corridos(ss)>=BOIS_PARA_CLASSIFICAR))
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(Controle_Torneio))==(dono_senha,estado_senha,bois_corridos,fila_geral,fila_disputa_atual,fila_disputa_proxima,eliminados_nesta_rodada,buffer_saida,tamanho_buffer_saida:={},{},{},<>,<>,<>,<>,<>,0;fase_atual,rodizio_atual,idx_correndo,torneio_iniciado,ultimo_rodizio_iniciado,racha_sem_finalizar,premiacao_individual,premiacao_vagas,premiacao_total_atual,colocacao,posicao_atual:=classificacao,<>,1,FALSE,FALSE,FALSE,VAQUEIROS*{0},0,PREMIO_TOTAL,{},0);
  Context_List_Initialisation(Machine(Controle_Torneio))==(skip);
  List_Initialisation(Machine(Controle_Torneio))==(fase_atual:=classificacao || rodizio_atual:=<> || idx_correndo:=1 || torneio_iniciado:=FALSE || ultimo_rodizio_iniciado:=FALSE || racha_sem_finalizar:=FALSE || premiacao_individual:=VAQUEIROS*{0} || premiacao_vagas:=0 || premiacao_total_atual:=PREMIO_TOTAL || colocacao:={} || posicao_atual:=0)
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Machine(Controle_Torneio))==(comprar_senha,desistir_senha,transferir_senha,iniciar_rodizio,iniciar_ultimo_rodizio,chamar_para_pista,correr_boi_classificacao,iniciar_fase_disputa,correr_boi_disputa,iniciar_nova_rodada_disputa,finalizar_sem_vencedor,rachar_sem_finalizar,rachar_premio,declarar_campeao_unico,observar_torneio_encerrado,consultar_status_senha,consultar_dono,total_senhas_vendidas,consultar_bois_derrubados);
...
THEORY ListInputX IS
  ...
  List_Input(Machine(Controle_Torneio),rachar_sem_finalizar)==(?);
...
THEORY ListOutputX IS
  ...
  List_Output(Machine(Controle_Torneio),rachar_sem_finalizar)==(?);
...
THEORY ListHeaderX IS
  ...
  List_Header(Machine(Controle_Torneio),rachar_sem_finalizar)==(rachar_sem_finalizar);
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Machine(Controle_Torneio),iniciar_fase_disputa)==(fase_atual = classificacao & rodizio_atual = <> & size(fila_geral) = 0 & size(fila_disputa_proxima)>0);
  ...
  List_Precondition(Machine(Controle_Torneio),finalizar_sem_vencedor)==(fase_atual = classificacao & rodizio_atual = <> & size(fila_geral) = 0 & ultimo_rodizio_iniciado = TRUE & size(fila_disputa_proxima) = 0);
  List_Precondition(Machine(Controle_Torneio),rachar_sem_finalizar)==(fase_atual = disputa & na_pista/:ran(estado_senha) & size(fila_disputa_atual) = 0 & #(xx,yy,zz).(xx: dom(estado_senha) & yy: dom(estado_senha) & zz: dom(estado_senha) & xx/=yy & xx/=zz & yy/=zz & estado_senha(xx) = classificada & estado_senha(yy) = classificada & estado_senha(zz) = classificada & xx: dom(dono_senha) & yy: dom(dono_senha) & zz: dom(dono_senha) & dono_senha(xx)/=dono_senha(yy) & dono_senha(xx)/=dono_senha(zz) & dono_senha(yy)/=dono_senha(zz)));
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(Controle_Torneio),declarar_campeao_unico)==(fase_atual = disputa & na_pista/:ran(estado_senha) & size(fila_disputa_atual) = 0 & #vv.(vv: VAQUEIROS & #xx.(xx: dom(estado_senha) & estado_senha(xx) = classificada) & !yy.(yy: dom(estado_senha) & yy: dom(dono_senha) & estado_senha(yy) = classificada => dono_senha(yy) = vv)) | @(vencedor,senhas_campeas).(vencedor: VAQUEIROS & !yy.(yy: dom(estado_senha) & estado_senha(yy) = classificada => dono_senha(yy) = vencedor) & senhas_campeas = dom(estado_senha|>{classificada}) ==> (btrue | fase_atual:=encerrado || estado_senha:=estado_senha<+dom(estado_senha|>{classificada})*{campea} || premiacao_individual:=premiacao_individual<+{vencedor|->premiacao_individual(vencedor)+premiacao_total_atual} || colocacao:=colocacao<+senhas_campeas*{1})));
  Expanded_List_Substitution(Machine(Controle_Torneio),rachar_premio)==(fase_atual = disputa & na_pista/:ran(estado_senha) & size(fila_disputa_atual) = 0 & #(xx,yy).(xx: dom(estado_senha) & yy: dom(estado_senha) & xx/=yy & estado_senha(xx) = classificada & estado_senha(yy) = classificada & xx: dom(dono_senha) & yy: dom(dono_senha) & dono_senha(xx)/=dono_senha(yy)) | @(senhas_vencedoras,valor_por_senha).(senhas_vencedoras = dom(estado_senha|>{classificada}) & valor_por_senha = premiacao_total_atual/card(senhas_vencedoras) ==> (btrue | fase_atual:=encerrado || estado_senha:=estado_senha<+dom(estado_senha|>{classificada})*{campea} || premiacao_individual:=%vq.(vq: VAQUEIROS | valor_por_senha*card(senhas_vencedoras/\dono_senha~[{vq}])) || colocacao:=colocacao<+senhas_vencedoras*{1})));
  Expanded_List_Substitution(Machine(Controle_Torneio),rachar_sem_finalizar)==(fase_atual = disputa & na_pista/:ran(estado_senha) & size(fila_disputa_atual) = 0 & #(xx,yy,zz).(xx: dom(estado_senha) & yy: dom(estado_senha) & zz: dom(estado_senha) & xx/=yy & xx/=zz & yy/=zz & estado_senha(xx) = classificada & estado_senha(yy) = classificada & estado_senha(zz) = classificada & xx: dom(dono_senha) & yy: dom(dono_senha) & zz: dom(dono_senha) & dono_senha(xx)/=dono_senha(yy) & dono_senha(xx)/=dono_senha(zz) & dono_senha(yy)/=dono_senha(zz)) | @(senhas_restantes,qtd).(senhas_restantes = dom(estado_senha|>{classificada}) & qtd = card(senhas_restantes) ==> racha_sem_finalizar,premiacao_vagas,posicao_atual:=TRUE,premiacao_total_atual/(2*(qtd-1)),qtd));
  Expanded_List_Substitution(Machine(Controle_Torneio),finalizar_sem_vencedor)==(fase_atual = classificacao & rodizio_atual = <> & size(fila_geral) = 0 & ultimo_rodizio_iniciado = TRUE & size(fila_disputa_proxima) = 0 | fase_atual:=encerrado);
  ...
  Expanded_List_Substitution(Machine(Controle_Torneio),correr_boi_disputa)==(fase_atual = disputa & size(fila_disputa_atual)>0 & jj: JULGAMENTO & estado_senha(fila_disputa_atual(1)) = na_pista & size(fila_disputa_atual)>0 & jj: JULGAMENTO | @ss.(ss = fila_disputa_atual(1) ==> (jj = zero_boi ==> estado_senha,fila_disputa_atual,eliminados_nesta_rodada:=estado_senha<+{ss|->eliminada},fila_disputa_atual\|/1,eliminados_nesta_rodada<-ss [] not(jj = zero_boi) ==> (jj = valeu_boi ==> (estado_senha,fila_disputa_atual:=estado_senha<+{ss|->classificada},fila_disputa_atual\|/1 || @(f1,f2).(f1: iseq(NAT1) & f2: iseq(NAT1) & fila_disputa_proxima = f1^f2 & !xx.(xx: ran(f1) => xx<ss) & !yy.(yy: ran(f2) => yy>ss) ==> fila_disputa_proxima:=f1^[ss]^f2)) [] not(jj = valeu_boi) ==> estado_senha,fila_disputa_atual:=estado_senha<+{ss|->classificada},fila_disputa_atual\|/1<-ss))) || (jj = zero_boi & racha_sem_finalizar = TRUE ==> @(ss,vaqueiro_eliminado).(ss = fila_disputa_atual(1) & vaqueiro_eliminado = dono_senha(ss) ==> premiacao_individual,premiacao_total_atual,colocacao,posicao_atual:=premiacao_individual<+{vaqueiro_eliminado|->premiacao_individual(vaqueiro_eliminado)+premiacao_vagas},premiacao_total_atual-premiacao_vagas,colocacao<+{ss|->posicao_atual},posicao_atual-1) [] not(jj = zero_boi & racha_sem_finalizar = TRUE) ==> skip));
  Expanded_List_Substitution(Machine(Controle_Torneio),iniciar_fase_disputa)==(fase_atual = classificacao & rodizio_atual = <> & size(fila_geral) = 0 & size(fila_disputa_proxima)>0 | fase_atual:=disputa);
  ...
  Expanded_List_Substitution(Machine(Controle_Torneio),iniciar_ultimo_rodizio)==(fase_atual = classificacao & rodizio_atual = <> & size(fila_geral)>0 & ultimo_rodizio_iniciado = FALSE & size(fila_geral)<=TAMANHO_RODIZIO & size(fila_geral)>0 | size(fila_geral)>TAMANHO_RODIZIO ==> buffer_saida,tamanho_buffer_saida,fila_geral:=fila_geral/|\TAMANHO_RODIZIO,TAMANHO_RODIZIO,fila_geral\|/TAMANHO_RODIZIO [] not(size(fila_geral)>TAMANHO_RODIZIO) ==> buffer_saida,tamanho_buffer_saida,fila_geral:=fila_geral,size(fila_geral),<> || rodizio_atual:=fila_geral || idx_correndo:=1 || ultimo_rodizio_iniciado:=TRUE || torneio_iniciado:=TRUE);
  Expanded_List_Substitution(Machine(Controle_Torneio),iniciar_rodizio)==(fase_atual = classificacao & rodizio_atual = <> & size(fila_geral)>0 & size(fila_geral)>TAMANHO_RODIZIO & ultimo_rodizio_iniciado = FALSE & size(fila_geral)>0 | size(fila_geral)>TAMANHO_RODIZIO ==> buffer_saida,tamanho_buffer_saida,fila_geral:=fila_geral/|\TAMANHO_RODIZIO,TAMANHO_RODIZIO,fila_geral\|/TAMANHO_RODIZIO [] not(size(fila_geral)>TAMANHO_RODIZIO) ==> buffer_saida,tamanho_buffer_saida,fila_geral:=fila_geral,size(fila_geral),<> || rodizio_atual:=fila_geral/|\TAMANHO_RODIZIO || idx_correndo:=1 || torneio_iniciado:=TRUE);
  ...
  List_Substitution(Machine(Controle_Torneio),iniciar_rodizio)==(extrair_lote_rodizio || rodizio_atual:=fila_geral/|\TAMANHO_RODIZIO || idx_correndo:=1 || torneio_iniciado:=TRUE);
  List_Substitution(Machine(Controle_Torneio),iniciar_ultimo_rodizio)==(extrair_lote_rodizio || rodizio_atual:=fila_geral || idx_correndo:=1 || ultimo_rodizio_iniciado:=TRUE || torneio_iniciado:=TRUE);
  ...
  List_Substitution(Machine(Controle_Torneio),correr_boi_disputa)==(processar_resultado_disputa(jj) || IF jj = zero_boi & racha_sem_finalizar = TRUE THEN ANY ss,vaqueiro_eliminado WHERE ss = fila_disputa_atual(1) & vaqueiro_eliminado = dono_senha(ss) THEN premiacao_individual(vaqueiro_eliminado):=premiacao_individual(vaqueiro_eliminado)+premiacao_vagas || premiacao_total_atual:=premiacao_total_atual-premiacao_vagas || colocacao(ss):=posicao_atual || posicao_atual:=posicao_atual-1 END END);
  ...
  List_Substitution(Machine(Controle_Torneio),rachar_sem_finalizar)==(ANY senhas_restantes,qtd WHERE senhas_restantes = dom(estado_senha|>{classificada}) & qtd = card(senhas_restantes) THEN racha_sem_finalizar:=TRUE || premiacao_vagas:=premiacao_total_atual/(2*(qtd-1)) || posicao_atual:=qtd END);
  List_Substitution(Machine(Controle_Torneio),rachar_premio)==(ANY senhas_vencedoras,valor_por_senha WHERE senhas_vencedoras = dom(estado_senha|>{classificada}) & valor_por_senha = premiacao_total_atual/card(senhas_vencedoras) THEN fase_atual:=encerrado || finalizar_torneio || premiacao_individual:=%vq.(vq: VAQUEIROS | valor_por_senha*card(senhas_vencedoras/\dono_senha~[{vq}])) || colocacao:=colocacao<+senhas_vencedoras*{1} END);
  List_Substitution(Machine(Controle_Torneio),declarar_campeao_unico)==(ANY vencedor,senhas_campeas WHERE vencedor: VAQUEIROS & !yy.(yy: dom(estado_senha) & estado_senha(yy) = classificada => dono_senha(yy) = vencedor) & senhas_campeas = dom(estado_senha|>{classificada}) THEN fase_atual:=encerrado || finalizar_torneio || premiacao_individual(vencedor):=premiacao_individual(vencedor)+premiacao_total_atual || colocacao:=colocacao<+senhas_campeas*{1} END);
...
THEORY ListANYVarX IS
  ...
  List_ANY_Var(Machine(Controle_Torneio),correr_boi_disputa)==((Var(ss) == btype(INTEGER,?,?)),(Var(vaqueiro_eliminado) == btype(INTEGER,?,?)));
  ...
  List_ANY_Var(Machine(Controle_Torneio),rachar_sem_finalizar)==((Var(senhas_restantes) == SetOf(btype(INTEGER,?,?))),(Var(qtd) == btype(INTEGER,?,?)));
  List_ANY_Var(Machine(Controle_Torneio),rachar_premio)==((Var(senhas_vencedoras) == SetOf(btype(INTEGER,?,?))),(Var(valor_por_senha) == btype(INTEGER,?,?)));
  List_ANY_Var(Machine(Controle_Torneio),declarar_campeao_unico)==((Var(vencedor) == btype(INTEGER,?,?)),(Var(senhas_campeas) == SetOf(btype(INTEGER,?,?))));
...
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(Controle_Torneio)) == (? | ? | posicao_atual,colocacao,premiacao_total_atual,premiacao_vagas,premiacao_individual,racha_sem_finalizar,ultimo_rodizio_iniciado,torneio_iniciado,idx_correndo,rodizio_atual,fase_atual | tamanho_buffer_saida,buffer_saida,eliminados_nesta_rodada,fila_disputa_proxima,fila_disputa_atual,fila_geral,bois_corridos,estado_senha,dono_senha | comprar_senha,desistir_senha,transferir_senha,iniciar_rodizio,iniciar_ultimo_rodizio,chamar_para_pista,correr_boi_classificacao,iniciar_fase_disputa,correr_boi_disputa,iniciar_nova_rodada_disputa,finalizar_sem_vencedor,rachar_sem_finalizar,rachar_premio,declarar_campeao_unico,observar_torneio_encerrado,consultar_status_senha,consultar_dono,total_senhas_vendidas,consultar_bois_derrubados | ? | seen(Machine(Contexto_Vaquejada)),included(Machine(Gerenciador_Senhas)) | ? | Controle_Torneio);
...
THEORY VariablesEnvX IS
  Variables(Machine(Controle_Torneio)) == (Type(dono_senha) == Mvl(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(estado_senha) == Mvl(SetOf(btype(INTEGER,?,?)*etype(STATUS_SENHA,?,?)));Type(bois_corridos) == Mvl(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(fila_geral) == Mvl(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(fila_disputa_atual) == Mvl(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(fila_disputa_proxima) == Mvl(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(eliminados_nesta_rodada) == Mvl(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(buffer_saida) == Mvl(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(tamanho_buffer_saida) == Mvl(btype(INTEGER,?,?));Type(posicao_atual) == Mvl(btype(INTEGER,?,?));Type(colocacao) == Mvl(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(premiacao_total_atual) == Mvl(btype(INTEGER,?,?));Type(premiacao_vagas) == Mvl(btype(INTEGER,?,?));Type(premiacao_individual) == Mvl(SetOf(btype(INTEGER,"[VAQUEIROS","]VAQUEIROS")*btype(INTEGER,0,MAXINT)));Type(racha_sem_finalizar) == Mvl(btype(BOOL,?,?));Type(ultimo_rodizio_iniciado) == Mvl(btype(BOOL,?,?));Type(torneio_iniciado) == Mvl(btype(BOOL,?,?));Type(idx_correndo) == Mvl(btype(INTEGER,?,?));Type(rodizio_atual) == Mvl(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(fase_atual) == Mvl(etype(FASES,?,?)))
END
&
