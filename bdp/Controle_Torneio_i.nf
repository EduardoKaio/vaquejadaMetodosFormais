...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Implementation(Controle_Torneio_i))==(dono_senha,estado_senha,bois_corridos,fila_geral,fila_disputa_atual,fila_disputa_proxima,eliminados_nesta_rodada,buffer_saida,tamanho_buffer_saida:={},{},{},<>,<>,<>,<>,<>,0;fase_atual_v:=classificacao;(0: INT | rodizio_size:=0);(1: INT | idx_correndo_v:=1);torneio_iniciado_v:=FALSE;ultimo_rodizio_iniciado_v:=FALSE;racha_sem_finalizar_v:=FALSE;(0: INT | premiacao_vagas_v:=0);(10000: INT | premiacao_total_atual_v:=10000);(0: INT | posicao_atual_v:=0);@ii.((0: INT | ii:=0);WHILE ii<=3 DO (ii: dom(rodizio_arr) & 0: INT | rodizio_arr:=rodizio_arr<+{ii|->0});(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 0..4 & !jj.(jj: 0..3 & jj<ii => rodizio_arr(jj) = 0) VARIANT 4-ii END);@ii.((0: INT | ii:=0);WHILE ii<=2 DO (ii: dom(premiacao_individual_arr) & 0: INT | premiacao_individual_arr:=premiacao_individual_arr<+{ii|->0});(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 0..3 & !jj.(jj: 0..2 & jj<ii => premiacao_individual_arr(jj) = 0) VARIANT 3-ii END);@ii.((0: INT | ii:=0);WHILE ii<=19 DO (ii: dom(colocacao_arr) & 0: INT | colocacao_arr:=colocacao_arr<+{ii|->0});(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 0..20 & !jj.(jj: 0..19 & jj<ii => colocacao_arr(jj) = 0) VARIANT 20-ii END));
  Context_List_Initialisation(Implementation(Controle_Torneio_i))==(skip);
  List_Initialisation(Implementation(Controle_Torneio_i))==(fase_atual_v:=classificacao;rodizio_size:=0;idx_correndo_v:=1;torneio_iniciado_v:=FALSE;ultimo_rodizio_iniciado_v:=FALSE;racha_sem_finalizar_v:=FALSE;premiacao_vagas_v:=0;premiacao_total_atual_v:=10000;posicao_atual_v:=0;VAR ii IN ii:=0;WHILE ii<=3 DO rodizio_arr(ii):=0;ii:=ii+1 INVARIANT ii: 0..4 & !jj.(jj: 0..3 & jj<ii => rodizio_arr(jj) = 0) VARIANT 4-ii END END;VAR ii IN ii:=0;WHILE ii<=2 DO premiacao_individual_arr(ii):=0;ii:=ii+1 INVARIANT ii: 0..3 & !jj.(jj: 0..2 & jj<ii => premiacao_individual_arr(jj) = 0) VARIANT 3-ii END END;VAR ii IN ii:=0;WHILE ii<=19 DO colocacao_arr(ii):=0;ii:=ii+1 INVARIANT ii: 0..20 & !jj.(jj: 0..19 & jj<ii => colocacao_arr(jj) = 0) VARIANT 20-ii END END)
END
&
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Implementation(Controle_Torneio_i),total_senhas_vendidas)==(btrue | @(ii,dono,count).((0: INT | count:=0);(1: INT | ii:=1);WHILE ii<=20 DO (ii: NAT1 & ii: dom(dono_senha) | dono:=dono_senha(ii));(dono/=0 ==> (count+1: INT & count: INT & 1: INT | count:=count+1) [] not(dono/=0) ==> skip);(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 1..21 & count = card({jj | jj: 1..ii-1 & jj: dom(dono_senha)}) VARIANT 21-ii END;(count: INT | res:=count)));
  ...
  Expanded_List_Substitution(Implementation(Controle_Torneio_i),declarar_campeao_unico)==(fase_atual = disputa & na_pista/:ran(estado_senha) & size(fila_disputa_atual) = 0 & #vv.(vv: VAQUEIROS & #xx.(xx: dom(estado_senha) & estado_senha(xx) = classificada) & !yy.(yy: dom(estado_senha) & yy: dom(dono_senha) & estado_senha(yy) = classificada => dono_senha(yy) = vv)) | @(ii,st,vq,cur_prize).((1: INT | ii:=1);WHILE ii<=20 DO (ii: NAT1 & ii: dom(estado_senha) | st:=estado_senha(ii));(st = classificada ==> ((ii: NAT1 & ii: dom(dono_senha) | vq:=dono_senha(ii));(ii-1: INT & ii: INT & 1: INT & ii-1: dom(colocacao_arr) | colocacao_arr:=colocacao_arr<+{ii-1|->1})) [] not(st = classificada) ==> skip);(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 1..21 & !jj.(jj: 1..ii-1 & jj: dom(estado_senha) & estado_senha(jj) = classificada => colocacao_arr(jj-1) = 1) VARIANT 21-ii END;(vq-1: INT & vq: INT & 1: INT & vq-1: dom(premiacao_individual_arr) | cur_prize:=premiacao_individual_arr(vq-1));(cur_prize+premiacao_total_atual_v: INT & cur_prize: INT & premiacao_total_atual_v: INT | cur_prize:=cur_prize+premiacao_total_atual_v);(vq-1: INT & vq: INT & 1: INT & vq-1: dom(premiacao_individual_arr) & cur_prize: INT | premiacao_individual_arr:=premiacao_individual_arr<+{vq-1|->cur_prize});fase_atual_v:=encerrado;(btrue | estado_senha:=estado_senha<+dom(estado_senha|>{classificada})*{campea})));
  Expanded_List_Substitution(Implementation(Controle_Torneio_i),rachar_premio)==(fase_atual = disputa & na_pista/:ran(estado_senha) & size(fila_disputa_atual) = 0 & #(xx,yy).(xx: dom(estado_senha) & yy: dom(estado_senha) & xx/=yy & estado_senha(xx) = classificada & estado_senha(yy) = classificada & xx: dom(dono_senha) & yy: dom(dono_senha) & dono_senha(xx)/=dono_senha(yy)) | @(ii,qtd,st,vq,prize_per_senha,cur_prize,old_premio).((0: INT | qtd:=0);(1: INT | ii:=1);WHILE ii<=20 DO (ii: NAT1 & ii: dom(estado_senha) | st:=estado_senha(ii));(st = classificada ==> (qtd+1: INT & qtd: INT & 1: INT | qtd:=qtd+1) [] not(st = classificada) ==> skip);(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 1..21 & qtd = card({jj | jj: 1..ii-1 & jj: dom(estado_senha) & estado_senha(jj) = classificada}) VARIANT 21-ii END;(qtd>0 ==> ((premiacao_total_atual_v/qtd: INT & premiacao_total_atual_v: INT & qtd: INT & not(qtd = 0) | prize_per_senha:=premiacao_total_atual_v/qtd);old_premio:=premiacao_individual_arr;(1: INT | ii:=1);WHILE ii<=20 DO (ii: NAT1 & ii: dom(estado_senha) | st:=estado_senha(ii));(st = classificada ==> ((ii: NAT1 & ii: dom(dono_senha) | vq:=dono_senha(ii));(vq-1: INT & vq: INT & 1: INT & vq-1: dom(premiacao_individual_arr) | cur_prize:=premiacao_individual_arr(vq-1));(cur_prize+prize_per_senha: INT & cur_prize: INT & prize_per_senha: INT | cur_prize:=cur_prize+prize_per_senha);(vq-1: INT & vq: INT & 1: INT & vq-1: dom(premiacao_individual_arr) & cur_prize: INT | premiacao_individual_arr:=premiacao_individual_arr<+{vq-1|->cur_prize});(ii-1: INT & ii: INT & 1: INT & ii-1: dom(colocacao_arr) | colocacao_arr:=colocacao_arr<+{ii-1|->1})) [] not(st = classificada) ==> skip);(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 1..21 & !vq2.(vq2: 1..3 => premiacao_individual_arr(vq2-1) = old_premio(vq2-1)+prize_per_senha*card({jj | jj: 1..ii-1 & jj: dom(estado_senha) & estado_senha(jj) = classificada & jj: dom(dono_senha) & dono_senha(jj) = vq2})) & !jj.(jj: 1..ii-1 & jj: dom(estado_senha) & estado_senha(jj) = classificada => colocacao_arr(jj-1) = 1) VARIANT 21-ii END) [] not(qtd>0) ==> skip);fase_atual_v:=encerrado;(btrue | estado_senha:=estado_senha<+dom(estado_senha|>{classificada})*{campea})));
  Expanded_List_Substitution(Implementation(Controle_Torneio_i),rachar_sem_finalizar)==(fase_atual = disputa & na_pista/:ran(estado_senha) & size(fila_disputa_atual) = 0 & #(xx,yy,zz).(xx: dom(estado_senha) & yy: dom(estado_senha) & zz: dom(estado_senha) & xx/=yy & xx/=zz & yy/=zz & estado_senha(xx) = classificada & estado_senha(yy) = classificada & estado_senha(zz) = classificada & xx: dom(dono_senha) & yy: dom(dono_senha) & zz: dom(dono_senha) & dono_senha(xx)/=dono_senha(yy) & dono_senha(xx)/=dono_senha(zz) & dono_senha(yy)/=dono_senha(zz)) | @(ii,qtd,st,div_base,prize_calc).((0: INT | qtd:=0);(1: INT | ii:=1);WHILE ii<=20 DO (ii: NAT1 & ii: dom(estado_senha) | st:=estado_senha(ii));(st = classificada ==> (qtd+1: INT & qtd: INT & 1: INT | qtd:=qtd+1) [] not(st = classificada) ==> skip);(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 1..21 & qtd = card({jj | jj: 1..ii-1 & jj: dom(estado_senha) & estado_senha(jj) = classificada}) VARIANT 21-ii END;racha_sem_finalizar_v:=TRUE;(qtd: INT | posicao_atual_v:=qtd);(qtd-1: INT & qtd: INT & 1: INT | div_base:=qtd-1);(2*div_base: INT & 2: INT & div_base: INT | div_base:=2*div_base);(div_base>0 ==> ((premiacao_total_atual_v/div_base: INT & premiacao_total_atual_v: INT & div_base: INT & not(div_base = 0) | prize_calc:=premiacao_total_atual_v/div_base);(prize_calc: INT | premiacao_vagas_v:=prize_calc)) [] not(div_base>0) ==> skip)));
  ...
  List_Substitution(Implementation(Controle_Torneio_i),iniciar_rodizio)==(VAR tam,ii,tmp_val IN extrair_lote_rodizio;tam <-- consultar_tamanho_lote_saida;ii:=0;WHILE ii<tam DO tmp_val <-- consultar_lote_saida(ii+1);rodizio_arr(ii):=tmp_val;ii:=ii+1 INVARIANT ii: 0..tam & !jj.(jj: 0..tam-1 & jj<ii => rodizio_arr(jj) = buffer_saida(jj+1)) VARIANT tam-ii END;rodizio_size:=tam;idx_correndo_v:=1;torneio_iniciado_v:=TRUE END);
  List_Substitution(Implementation(Controle_Torneio_i),iniciar_ultimo_rodizio)==(VAR tam,ii,tmp_val IN extrair_lote_rodizio;tam <-- consultar_tamanho_lote_saida;ii:=0;WHILE ii<tam DO tmp_val <-- consultar_lote_saida(ii+1);rodizio_arr(ii):=tmp_val;ii:=ii+1 INVARIANT ii: 0..tam & !jj.(jj: 0..tam-1 & jj<ii => rodizio_arr(jj) = buffer_saida(jj+1)) VARIANT tam-ii END;rodizio_size:=tam;idx_correndo_v:=1;ultimo_rodizio_iniciado_v:=TRUE;torneio_iniciado_v:=TRUE END);
  List_Substitution(Implementation(Controle_Torneio_i),chamar_para_pista)==(atualizar_status(ss,na_pista));
  List_Substitution(Implementation(Controle_Torneio_i),correr_boi_classificacao)==(VAR bc,lim,ii,prox,tmp_val,nbc,old_rodizio IN registrar_julgamento_classificacao(ss,jj);bc <-- get_bois_corridos(ss);nbc:=bc+1;IF jj = zero_boi or (jj = valeu_boi & nbc = BOIS_PARA_CLASSIFICAR) THEN old_rodizio:=rodizio_arr;ii:=idx_correndo_v-1;lim:=rodizio_size-1;WHILE ii<lim DO prox:=ii+1;tmp_val:=rodizio_arr(prox);rodizio_arr(ii):=tmp_val;ii:=ii+1 INVARIANT ii: idx_correndo_v-1..lim & !jj2.(jj2: idx_correndo_v-1..lim & jj2<ii => rodizio_arr(jj2) = old_rodizio(jj2+1)) & !jj2.(jj2: idx_correndo_v-1..lim & jj2>=ii => rodizio_arr(jj2) = old_rodizio(jj2)) VARIANT lim-ii END;rodizio_size:=rodizio_size-1;IF rodizio_size = 0 THEN idx_correndo_v:=1 ELSIF idx_correndo_v>rodizio_size THEN idx_correndo_v:=1 END ELSE IF idx_correndo_v<rodizio_size THEN idx_correndo_v:=idx_correndo_v+1 ELSE idx_correndo_v:=1 END END END);
  ...
  List_Substitution(Implementation(Controle_Torneio_i),rachar_sem_finalizar)==(VAR ii,qtd,st,div_base,prize_calc IN qtd:=0;ii:=1;WHILE ii<=20 DO st <-- get_estado_senha(ii);IF st = classificada THEN qtd:=qtd+1 END;ii:=ii+1 INVARIANT ii: 1..21 & qtd = card({jj | jj: 1..ii-1 & jj: dom(estado_senha) & estado_senha(jj) = classificada}) VARIANT 21-ii END;racha_sem_finalizar_v:=TRUE;posicao_atual_v:=qtd;div_base:=qtd-1;div_base:=2*div_base;IF div_base>0 THEN prize_calc:=premiacao_total_atual_v/div_base;premiacao_vagas_v:=prize_calc END END);
  List_Substitution(Implementation(Controle_Torneio_i),rachar_premio)==(VAR ii,qtd,st,vq,prize_per_senha,cur_prize,old_premio IN qtd:=0;ii:=1;WHILE ii<=20 DO st <-- get_estado_senha(ii);IF st = classificada THEN qtd:=qtd+1 END;ii:=ii+1 INVARIANT ii: 1..21 & qtd = card({jj | jj: 1..ii-1 & jj: dom(estado_senha) & estado_senha(jj) = classificada}) VARIANT 21-ii END;IF qtd>0 THEN prize_per_senha:=premiacao_total_atual_v/qtd;old_premio:=premiacao_individual_arr;ii:=1;WHILE ii<=20 DO st <-- get_estado_senha(ii);IF st = classificada THEN vq <-- get_dono_senha(ii);cur_prize:=premiacao_individual_arr(vq-1);cur_prize:=cur_prize+prize_per_senha;premiacao_individual_arr(vq-1):=cur_prize;colocacao_arr(ii-1):=1 END;ii:=ii+1 INVARIANT ii: 1..21 & !vq2.(vq2: 1..3 => premiacao_individual_arr(vq2-1) = old_premio(vq2-1)+prize_per_senha*card({jj | jj: 1..ii-1 & jj: dom(estado_senha) & estado_senha(jj) = classificada & jj: dom(dono_senha) & dono_senha(jj) = vq2})) & !jj.(jj: 1..ii-1 & jj: dom(estado_senha) & estado_senha(jj) = classificada => colocacao_arr(jj-1) = 1) VARIANT 21-ii END END;fase_atual_v:=encerrado;finalizar_torneio END);
  List_Substitution(Implementation(Controle_Torneio_i),declarar_campeao_unico)==(VAR ii,st,vq,cur_prize IN ii:=1;WHILE ii<=20 DO st <-- get_estado_senha(ii);IF st = classificada THEN vq <-- get_dono_senha(ii);colocacao_arr(ii-1):=1 END;ii:=ii+1 INVARIANT ii: 1..21 & !jj.(jj: 1..ii-1 & jj: dom(estado_senha) & estado_senha(jj) = classificada => colocacao_arr(jj-1) = 1) VARIANT 21-ii END;cur_prize:=premiacao_individual_arr(vq-1);cur_prize:=cur_prize+premiacao_total_atual_v;premiacao_individual_arr(vq-1):=cur_prize;fase_atual_v:=encerrado;finalizar_torneio END);
  ...
  List_Substitution(Implementation(Controle_Torneio_i),total_senhas_vendidas)==(VAR ii,dono,count IN count:=0;ii:=1;WHILE ii<=20 DO dono <-- get_dono_senha(ii);IF dono/=0 THEN count:=count+1 END;ii:=ii+1 INVARIANT ii: 1..21 & count = card({jj | jj: 1..ii-1 & jj: dom(dono_senha)}) VARIANT 21-ii END;res:=count END);
...
THEORY VariablesLocEnvX IS
  ...
  Variables_Loc(Implementation(Controle_Torneio_i),correr_boi_classificacao, 1) == (Type(bc) == Lvl(btype(INTEGER,?,?));Type(lim) == Lvl(btype(INTEGER,?,?));Type(ii) == Lvl(btype(INTEGER,?,?));Type(prox) == Lvl(btype(INTEGER,?,?));Type(tmp_val) == Lvl(btype(INTEGER,?,?));Type(nbc) == Lvl(btype(INTEGER,?,?));Type(old_rodizio) == Lvl(SetOf(btype(INTEGER,0,3)*btype(INTEGER,0,MAXINT))));
  ...
  Variables_Loc(Implementation(Controle_Torneio_i),rachar_premio, 1) == (Type(ii) == Lvl(btype(INTEGER,?,?));Type(qtd) == Lvl(btype(INTEGER,?,?));Type(st) == Lvl(etype(STATUS_SENHA,?,?));Type(vq) == Lvl(btype(INTEGER,?,?));Type(prize_per_senha) == Lvl(btype(INTEGER,?,?));Type(cur_prize) == Lvl(btype(INTEGER,?,?));Type(old_premio) == Lvl(SetOf(btype(INTEGER,0,2)*btype(INTEGER,0,MAXINT))));
