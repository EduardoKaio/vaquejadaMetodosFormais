...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Implementation(Gerenciador_Senhas_i))==(@ii.((0: INT | ii:=0);WHILE ii<=19 DO (ii: dom(dono_senha_arr) & 0: INT | dono_senha_arr:=dono_senha_arr<+{ii|->0});(ii: dom(estado_senha_arr) | estado_senha_arr:=estado_senha_arr<+{ii|->na_espera});(ii: dom(bois_corridos_arr) & 0: INT | bois_corridos_arr:=bois_corridos_arr<+{ii|->0});(ii: dom(fila_geral_arr) & 0: INT | fila_geral_arr:=fila_geral_arr<+{ii|->0});(ii: dom(fila_disputa_atual_arr) & 0: INT | fila_disputa_atual_arr:=fila_disputa_atual_arr<+{ii|->0});(ii: dom(fila_disputa_proxima_arr) & 0: INT | fila_disputa_proxima_arr:=fila_disputa_proxima_arr<+{ii|->0});(ii: dom(eliminados_arr) & 0: INT | eliminados_arr:=eliminados_arr<+{ii|->0});(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 0..20 & !jj.(jj: 0..19 & jj<ii => dono_senha_arr(jj) = 0 & estado_senha_arr(jj) = na_espera & bois_corridos_arr(jj) = 0 & fila_geral_arr(jj) = 0 & fila_disputa_atual_arr(jj) = 0 & fila_disputa_proxima_arr(jj) = 0 & eliminados_arr(jj) = 0) VARIANT 20-ii END);@ii.((0: INT | ii:=0);WHILE ii<=3 DO (ii: dom(lote_saida_arr) & 0: INT | lote_saida_arr:=lote_saida_arr<+{ii|->0});(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 0..4 & !jj.(jj: 0..3 & jj<ii => lote_saida_arr(jj) = 0) VARIANT 4-ii END);(0: INT | lote_saida_size:=0);(0: INT | fila_geral_fim:=0);(0: INT | fila_disputa_atual_fim:=0);(0: INT | fila_disputa_proxima_fim:=0);(0: INT | eliminados_fim:=0));
  Context_List_Initialisation(Implementation(Gerenciador_Senhas_i))==(skip);
  List_Initialisation(Implementation(Gerenciador_Senhas_i))==(VAR ii IN ii:=0;WHILE ii<=19 DO dono_senha_arr(ii):=0;estado_senha_arr(ii):=na_espera;bois_corridos_arr(ii):=0;fila_geral_arr(ii):=0;fila_disputa_atual_arr(ii):=0;fila_disputa_proxima_arr(ii):=0;eliminados_arr(ii):=0;ii:=ii+1 INVARIANT ii: 0..20 & !jj.(jj: 0..19 & jj<ii => dono_senha_arr(jj) = 0 & estado_senha_arr(jj) = na_espera & bois_corridos_arr(jj) = 0 & fila_geral_arr(jj) = 0 & fila_disputa_atual_arr(jj) = 0 & fila_disputa_proxima_arr(jj) = 0 & eliminados_arr(jj) = 0) VARIANT 20-ii END END;VAR ii IN ii:=0;WHILE ii<=3 DO lote_saida_arr(ii):=0;ii:=ii+1 INVARIANT ii: 0..4 & !jj.(jj: 0..3 & jj<ii => lote_saida_arr(jj) = 0) VARIANT 4-ii END END;lote_saida_size:=0;fila_geral_fim:=0;fila_disputa_atual_fim:=0;fila_disputa_proxima_fim:=0;eliminados_fim:=0)
END
&
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Implementation(Gerenciador_Senhas_i),finalizar_torneio)==(btrue | @(ii,st,old_estado).(old_estado:=estado_senha_arr;(0: INT | ii:=0);WHILE ii<=19 DO (ii: dom(estado_senha_arr) | st:=estado_senha_arr(ii));(st = classificada ==> (ii: dom(estado_senha_arr) | estado_senha_arr:=estado_senha_arr<+{ii|->campea}) [] not(st = classificada) ==> skip);(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 0..20 & !jj.(jj: 0..19 & jj<ii & old_estado(jj) = classificada => estado_senha_arr(jj) = campea) & !jj.(jj: 0..19 & jj<ii & old_estado(jj)/=classificada => estado_senha_arr(jj) = old_estado(jj)) VARIANT 20-ii END));
  Expanded_List_Substitution(Implementation(Gerenciador_Senhas_i),avancar_rodada_disputa)==(size(fila_disputa_atual) = 0 & (size(fila_disputa_proxima)>0 or size(eliminados_nesta_rodada)>0) | fila_disputa_proxima_fim>0 ==> (@(ii,tmp_val).((0: INT | ii:=0);WHILE ii<fila_disputa_proxima_fim DO (ii: dom(fila_disputa_proxima_arr) | tmp_val:=fila_disputa_proxima_arr(ii));(ii: dom(fila_disputa_atual_arr) & tmp_val: INT | fila_disputa_atual_arr:=fila_disputa_atual_arr<+{ii|->tmp_val});(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 0..fila_disputa_proxima_fim & !jj.(jj: 0..fila_disputa_proxima_fim-1 & jj<ii => fila_disputa_atual_arr(jj) = fila_disputa_proxima_arr(jj)) VARIANT fila_disputa_proxima_fim-ii END);(fila_disputa_proxima_fim: INT | fila_disputa_atual_fim:=fila_disputa_proxima_fim);(0: INT | fila_disputa_proxima_fim:=0);(0: INT | eliminados_fim:=0)) [] not(fila_disputa_proxima_fim>0) ==> (@(ii,ss_val,idx).((0: INT | ii:=0);WHILE ii<eliminados_fim DO (ii: dom(eliminados_arr) | ss_val:=eliminados_arr(ii));(ii: dom(fila_disputa_atual_arr) & ss_val: INT | fila_disputa_atual_arr:=fila_disputa_atual_arr<+{ii|->ss_val});(ss_val-1: INT & ss_val: INT & 1: INT | idx:=ss_val-1);(idx: dom(estado_senha_arr) | estado_senha_arr:=estado_senha_arr<+{idx|->classificada});(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 0..eliminados_fim & !jj.(jj: 0..eliminados_fim-1 & jj<ii => fila_disputa_atual_arr(jj) = eliminados_arr(jj)) & !jj.(jj: 0..eliminados_fim-1 & jj<ii => estado_senha_arr(eliminados_arr(jj)-1) = classificada) VARIANT eliminados_fim-ii END);(eliminados_fim: INT | fila_disputa_atual_fim:=eliminados_fim);(0: INT | eliminados_fim:=0)));
  Expanded_List_Substitution(Implementation(Gerenciador_Senhas_i),processar_resultado_disputa)==(size(fila_disputa_atual)>0 & jj: JULGAMENTO | @(ss_val,idx,pos,ii,prev,prox,tmp_val,lim,old_atual,old_proxima).((0: dom(fila_disputa_atual_arr) | ss_val:=fila_disputa_atual_arr(0));(ss_val-1: INT & ss_val: INT & 1: INT | idx:=ss_val-1);old_atual:=fila_disputa_atual_arr;(fila_disputa_atual_fim-1: INT & fila_disputa_atual_fim: INT & 1: INT | lim:=fila_disputa_atual_fim-1);(0: INT | ii:=0);WHILE ii<lim DO (ii+1: INT & ii: INT & 1: INT | prox:=ii+1);(prox: dom(fila_disputa_atual_arr) | tmp_val:=fila_disputa_atual_arr(prox));(ii: dom(fila_disputa_atual_arr) & tmp_val: INT | fila_disputa_atual_arr:=fila_disputa_atual_arr<+{ii|->tmp_val});(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 0..fila_disputa_atual_fim-1 & !jj2.(jj2: 0..fila_disputa_atual_fim-1 & jj2<ii => fila_disputa_atual_arr(jj2) = old_atual(jj2+1)) & !jj2.(jj2: 0..fila_disputa_atual_fim-1 & jj2>=ii => fila_disputa_atual_arr(jj2) = old_atual(jj2)) VARIANT fila_disputa_atual_fim-1-ii END;(fila_disputa_atual_fim-1: INT & fila_disputa_atual_fim: INT & 1: INT | fila_disputa_atual_fim:=fila_disputa_atual_fim-1);(jj = zero_boi ==> ((idx: dom(estado_senha_arr) | estado_senha_arr:=estado_senha_arr<+{idx|->eliminada});(eliminados_fim: dom(eliminados_arr) & ss_val: INT | eliminados_arr:=eliminados_arr<+{eliminados_fim|->ss_val});(eliminados_fim+1: INT & eliminados_fim: INT & 1: INT | eliminados_fim:=eliminados_fim+1)) [] not(jj = zero_boi) ==> (jj = valeu_boi ==> ((idx: dom(estado_senha_arr) | estado_senha_arr:=estado_senha_arr<+{idx|->classificada});(0: INT | pos:=0);(pos<fila_disputa_proxima_fim ==> ((fila_disputa_proxima_arr(pos): INT & pos: dom(fila_disputa_proxima_arr) | tmp_val:=fila_disputa_proxima_arr(pos));WHILE pos<fila_disputa_proxima_fim & tmp_val<ss_val DO (pos+1: INT & pos: INT & 1: INT | pos:=pos+1);(pos<fila_disputa_proxima_fim ==> (fila_disputa_proxima_arr(pos): INT & pos: dom(fila_disputa_proxima_arr) | tmp_val:=fila_disputa_proxima_arr(pos)) [] not(pos<fila_disputa_proxima_fim) ==> skip) INVARIANT pos: 0..fila_disputa_proxima_fim & !jj2.(jj2: 0..fila_disputa_proxima_fim-1 & jj2<pos => fila_disputa_proxima_arr(jj2)<ss_val) & (pos<fila_disputa_proxima_fim => tmp_val = fila_disputa_proxima_arr(pos)) VARIANT fila_disputa_proxima_fim-pos END) [] not(pos<fila_disputa_proxima_fim) ==> skip);old_proxima:=fila_disputa_proxima_arr;(fila_disputa_proxima_fim: INT | ii:=fila_disputa_proxima_fim);WHILE ii>pos DO (ii-1: INT & ii: INT & 1: INT | prev:=ii-1);(fila_disputa_proxima_arr(prev): INT & prev: dom(fila_disputa_proxima_arr) | tmp_val:=fila_disputa_proxima_arr(prev));(ii: dom(fila_disputa_proxima_arr) & tmp_val: INT | fila_disputa_proxima_arr:=fila_disputa_proxima_arr<+{ii|->tmp_val});(ii-1: INT & ii: INT & 1: INT | ii:=ii-1) INVARIANT ii: pos..fila_disputa_proxima_fim & !jj2.(jj2: 0..fila_disputa_proxima_fim & jj2<=ii => fila_disputa_proxima_arr(jj2) = old_proxima(jj2)) & !jj2.(jj2: 0..fila_disputa_proxima_fim & jj2>ii => fila_disputa_proxima_arr(jj2) = old_proxima(jj2-1)) VARIANT ii-pos END;(pos: dom(fila_disputa_proxima_arr) & ss_val: INT | fila_disputa_proxima_arr:=fila_disputa_proxima_arr<+{pos|->ss_val});(fila_disputa_proxima_fim+1: INT & fila_disputa_proxima_fim: INT & 1: INT | fila_disputa_proxima_fim:=fila_disputa_proxima_fim+1)) [] not(jj = valeu_boi) ==> ((idx: dom(estado_senha_arr) | estado_senha_arr:=estado_senha_arr<+{idx|->classificada});(fila_disputa_atual_fim: dom(fila_disputa_atual_arr) & ss_val: INT | fila_disputa_atual_arr:=fila_disputa_atual_arr<+{fila_disputa_atual_fim|->ss_val});(fila_disputa_atual_fim+1: INT & fila_disputa_atual_fim: INT & 1: INT | fila_disputa_atual_fim:=fila_disputa_atual_fim+1))))));
  Expanded_List_Substitution(Implementation(Gerenciador_Senhas_i),registrar_julgamento_classificacao)==(ss: NAT1 & ss: dom(estado_senha) & jj: JULGAMENTO | @(idx,bc,pos,ii,prev,tmp_val,nbc,old_proxima).((ss-1: INT & ss: INT & 1: INT | idx:=ss-1);(jj = zero_boi ==> (idx: dom(estado_senha_arr) | estado_senha_arr:=estado_senha_arr<+{idx|->eliminada}) [] not(jj = zero_boi) ==> (jj = valeu_boi ==> ((idx: dom(bois_corridos_arr) | bc:=bois_corridos_arr(idx));(bc+1: INT & bc: INT & 1: INT | nbc:=bc+1);(idx: dom(bois_corridos_arr) & nbc: INT | bois_corridos_arr:=bois_corridos_arr<+{idx|->nbc});(nbc = BOIS_PARA_CLASSIFICAR ==> ((idx: dom(estado_senha_arr) | estado_senha_arr:=estado_senha_arr<+{idx|->classificada});(0: INT | pos:=0);(pos<fila_disputa_proxima_fim ==> ((pos: dom(fila_disputa_proxima_arr) | tmp_val:=fila_disputa_proxima_arr(pos));WHILE pos<fila_disputa_proxima_fim & tmp_val<ss DO (pos+1: INT & pos: INT & 1: INT | pos:=pos+1);(pos<fila_disputa_proxima_fim ==> (fila_disputa_proxima_arr(pos): INT & pos: dom(fila_disputa_proxima_arr) | tmp_val:=fila_disputa_proxima_arr(pos)) [] not(pos<fila_disputa_proxima_fim) ==> skip) INVARIANT pos: 0..fila_disputa_proxima_fim & !jj2.(jj2: 0..fila_disputa_proxima_fim-1 & jj2<pos => fila_disputa_proxima_arr(jj2)<ss) & (pos<fila_disputa_proxima_fim => tmp_val = fila_disputa_proxima_arr(pos)) VARIANT fila_disputa_proxima_fim-pos END) [] not(pos<fila_disputa_proxima_fim) ==> skip);old_proxima:=fila_disputa_proxima_arr;ii:=fila_disputa_proxima_fim;WHILE ii>pos DO (ii-1: INT & ii: INT & 1: INT | prev:=ii-1);(fila_disputa_proxima_arr(prev): INT & prev: dom(fila_disputa_proxima_arr) | tmp_val:=fila_disputa_proxima_arr(prev));(ii: dom(fila_disputa_proxima_arr) & tmp_val: INT | fila_disputa_proxima_arr:=fila_disputa_proxima_arr<+{ii|->tmp_val});(ii-1: INT & ii: INT & 1: INT | ii:=ii-1) INVARIANT ii: pos..fila_disputa_proxima_fim & !jj2.(jj2: 0..fila_disputa_proxima_fim & jj2<=ii => fila_disputa_proxima_arr(jj2) = old_proxima(jj2)) & !jj2.(jj2: 0..fila_disputa_proxima_fim & jj2>ii => fila_disputa_proxima_arr(jj2) = old_proxima(jj2-1)) VARIANT ii-pos END;(pos: dom(fila_disputa_proxima_arr) & ss: INT | fila_disputa_proxima_arr:=fila_disputa_proxima_arr<+{pos|->ss});(fila_disputa_proxima_fim+1: INT & fila_disputa_proxima_fim: INT & 1: INT | fila_disputa_proxima_fim:=fila_disputa_proxima_fim+1)) [] not(nbc = BOIS_PARA_CLASSIFICAR) ==> (idx: dom(estado_senha_arr) | estado_senha_arr:=estado_senha_arr<+{idx|->na_espera}))) [] not(jj = valeu_boi) ==> (idx: dom(estado_senha_arr) | estado_senha_arr:=estado_senha_arr<+{idx|->na_espera})))));
  Expanded_List_Substitution(Implementation(Gerenciador_Senhas_i),extrair_lote_rodizio)==(size(fila_geral)>0 | @(qtd,ii,offset,tmp_val,lim,old_geral).(fila_geral_fim>TAMANHO_RODIZIO ==> qtd:=TAMANHO_RODIZIO [] not(fila_geral_fim>TAMANHO_RODIZIO) ==> (fila_geral_fim: INT | qtd:=fila_geral_fim);old_geral:=fila_geral_arr;(0: INT | ii:=0);WHILE ii<qtd DO (ii: dom(fila_geral_arr) | tmp_val:=fila_geral_arr(ii));(ii: dom(lote_saida_arr) & tmp_val: INT | lote_saida_arr:=lote_saida_arr<+{ii|->tmp_val});(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 0..qtd & !jj.(jj: 0..qtd-1 & jj<ii => lote_saida_arr(jj) = old_geral(jj)) VARIANT qtd-ii END;(qtd: INT | lote_saida_size:=qtd);(fila_geral_fim-qtd: INT & fila_geral_fim: INT & qtd: INT | lim:=fila_geral_fim-qtd);(0: INT | ii:=0);WHILE ii<lim DO (ii+qtd: INT & ii: INT & qtd: INT | offset:=ii+qtd);(fila_geral_arr(offset): INT & offset: dom(fila_geral_arr) | tmp_val:=fila_geral_arr(offset));(ii: dom(fila_geral_arr) & tmp_val: INT | fila_geral_arr:=fila_geral_arr<+{ii|->tmp_val});(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: 0..fila_geral_fim-qtd & !jj.(jj: 0..fila_geral_fim-qtd-1 & jj<ii => fila_geral_arr(jj) = old_geral(jj+qtd)) & !jj.(jj: 0..fila_geral_fim-qtd-1 & jj>=ii => fila_geral_arr(jj) = old_geral(jj)) VARIANT fila_geral_fim-qtd-ii END;(fila_geral_fim-qtd: INT & fila_geral_fim: INT & qtd: INT | fila_geral_fim:=fila_geral_fim-qtd)));
  ...
  Expanded_List_Substitution(Implementation(Gerenciador_Senhas_i),cancelar_registro_senha)==(ss: SENHAS & ss: NAT1 & ss: dom(dono_senha) & ss: ran(fila_geral) | @(pos,ii,idx,prox,tmp_val,lim,old_geral).((ss-1: INT & ss: INT & 1: INT | idx:=ss-1);(idx: dom(dono_senha_arr) & 0: INT | dono_senha_arr:=dono_senha_arr<+{idx|->0});(idx: dom(estado_senha_arr) | estado_senha_arr:=estado_senha_arr<+{idx|->na_espera});(idx: dom(bois_corridos_arr) & 0: INT | bois_corridos_arr:=bois_corridos_arr<+{idx|->0});(0: INT | pos:=0);(pos<fila_geral_fim ==> ((pos: dom(fila_geral_arr) | tmp_val:=fila_geral_arr(pos));WHILE pos<fila_geral_fim & tmp_val/=ss DO (pos+1: INT & pos: INT & 1: INT | pos:=pos+1);(pos<fila_geral_fim ==> (fila_geral_arr(pos): INT & pos: dom(fila_geral_arr) | tmp_val:=fila_geral_arr(pos)) [] not(pos<fila_geral_fim) ==> skip) INVARIANT pos: 0..fila_geral_fim & !jj.(jj: 0..fila_geral_fim-1 & jj<pos => fila_geral_arr(jj)/=ss) & (pos<fila_geral_fim => tmp_val = fila_geral_arr(pos)) VARIANT fila_geral_fim-pos END) [] not(pos<fila_geral_fim) ==> skip);old_geral:=fila_geral_arr;(fila_geral_fim-1: INT & fila_geral_fim: INT & 1: INT | lim:=fila_geral_fim-1);ii:=pos;WHILE ii<lim DO (ii+1: INT & ii: INT & 1: INT | prox:=ii+1);(fila_geral_arr(prox): INT & prox: dom(fila_geral_arr) | tmp_val:=fila_geral_arr(prox));(ii: dom(fila_geral_arr) & tmp_val: INT | fila_geral_arr:=fila_geral_arr<+{ii|->tmp_val});(ii+1: INT & ii: INT & 1: INT | ii:=ii+1) INVARIANT ii: pos..fila_geral_fim-1 & !jj.(jj: pos..fila_geral_fim-1 & jj<ii => fila_geral_arr(jj) = old_geral(jj+1)) & !jj.(jj: pos..fila_geral_fim-1 & jj>=ii => fila_geral_arr(jj) = old_geral(jj)) VARIANT fila_geral_fim-1-ii END;(fila_geral_fim-1: INT & fila_geral_fim: INT & 1: INT | fila_geral_fim:=fila_geral_fim-1)));
  ...
  Expanded_List_Substitution(Implementation(Gerenciador_Senhas_i),cadastrar_senha)==(vv: VAQUEIROS & vv: NAT1 & ss: SENHAS & ss: NAT1 & torneio_comecou: BOOL & ultima_senha: NAT & ss/:dom(dono_senha) & card(dom(dono_senha))<TOTAL_SENHAS & card(dom(dono_senha|>{vv}))<LIMITE_SENHAS_VAQUEIRO | @(pos,ii,idx,prev,tmp_val,old_geral).((ss-1: INT & ss: INT & 1: INT | idx:=ss-1);(idx: dom(dono_senha_arr) & vv: INT | dono_senha_arr:=dono_senha_arr<+{idx|->vv});(idx: dom(estado_senha_arr) | estado_senha_arr:=estado_senha_arr<+{idx|->na_espera});(idx: dom(bois_corridos_arr) & 0: INT | bois_corridos_arr:=bois_corridos_arr<+{idx|->0});(torneio_comecou = FALSE ==> ((0: INT | pos:=0);(pos<fila_geral_fim ==> ((pos: dom(fila_geral_arr) | tmp_val:=fila_geral_arr(pos));WHILE pos<fila_geral_fim & tmp_val<ss DO (pos+1: INT & pos: INT & 1: INT | pos:=pos+1);(pos<fila_geral_fim ==> (fila_geral_arr(pos): INT & pos: dom(fila_geral_arr) | tmp_val:=fila_geral_arr(pos)) [] not(pos<fila_geral_fim) ==> skip) INVARIANT pos: 0..fila_geral_fim & !jj.(jj: 0..fila_geral_fim-1 & jj<pos => fila_geral_arr(jj)<ss) & (pos<fila_geral_fim => tmp_val = fila_geral_arr(pos)) VARIANT fila_geral_fim-pos END) [] not(pos<fila_geral_fim) ==> skip);old_geral:=fila_geral_arr;ii:=fila_geral_fim;WHILE ii>pos DO (ii-1: INT & ii: INT & 1: INT | prev:=ii-1);(fila_geral_arr(prev): INT & prev: dom(fila_geral_arr) | tmp_val:=fila_geral_arr(prev));(ii: dom(fila_geral_arr) & tmp_val: INT | fila_geral_arr:=fila_geral_arr<+{ii|->tmp_val});(ii-1: INT & ii: INT & 1: INT | ii:=ii-1) INVARIANT ii: pos..fila_geral_fim & !jj.(jj: 0..fila_geral_fim & jj<=ii => fila_geral_arr(jj) = old_geral(jj)) & !jj.(jj: 0..fila_geral_fim & jj>ii => fila_geral_arr(jj) = old_geral(jj-1)) VARIANT ii-pos END;(pos: dom(fila_geral_arr) & ss: INT | fila_geral_arr:=fila_geral_arr<+{pos|->ss});(fila_geral_fim+1: INT & fila_geral_fim: INT & 1: INT | fila_geral_fim:=fila_geral_fim+1)) [] not(torneio_comecou = FALSE) ==> (ss<ultima_senha ==> ((fila_geral_fim: dom(fila_geral_arr) & ss: INT | fila_geral_arr:=fila_geral_arr<+{fila_geral_fim|->ss});(fila_geral_fim+1: INT & fila_geral_fim: INT & 1: INT | fila_geral_fim:=fila_geral_fim+1)) [] not(ss<ultima_senha) ==> ((0: INT | pos:=0);(pos<fila_geral_fim ==> ((fila_geral_arr(pos): INT & pos: dom(fila_geral_arr) | tmp_val:=fila_geral_arr(pos));WHILE pos<fila_geral_fim & tmp_val<ss & tmp_val>ultima_senha DO (pos+1: INT & pos: INT & 1: INT | pos:=pos+1);(pos<fila_geral_fim ==> (fila_geral_arr(pos): INT & pos: dom(fila_geral_arr) | tmp_val:=fila_geral_arr(pos)) [] not(pos<fila_geral_fim) ==> skip) INVARIANT pos: 0..fila_geral_fim & !jj.(jj: 0..fila_geral_fim-1 & jj<pos => fila_geral_arr(jj)<ss & fila_geral_arr(jj)>ultima_senha) & (pos<fila_geral_fim => tmp_val = fila_geral_arr(pos)) VARIANT fila_geral_fim-pos END) [] not(pos<fila_geral_fim) ==> skip);old_geral:=fila_geral_arr;(fila_geral_fim: INT | ii:=fila_geral_fim);WHILE ii>pos DO (ii-1: INT & ii: INT & 1: INT | prev:=ii-1);(fila_geral_arr(prev): INT & prev: dom(fila_geral_arr) | tmp_val:=fila_geral_arr(prev));(ii: dom(fila_geral_arr) & tmp_val: INT | fila_geral_arr:=fila_geral_arr<+{ii|->tmp_val});(ii-1: INT & ii: INT & 1: INT | ii:=ii-1) INVARIANT ii: pos..fila_geral_fim & !jj.(jj: 0..fila_geral_fim & jj<=ii => fila_geral_arr(jj) = old_geral(jj)) & !jj.(jj: 0..fila_geral_fim & jj>ii => fila_geral_arr(jj) = old_geral(jj-1)) VARIANT ii-pos END;(pos: dom(fila_geral_arr) & ss: INT | fila_geral_arr:=fila_geral_arr<+{pos|->ss});(fila_geral_fim+1: INT & fila_geral_fim: INT & 1: INT | fila_geral_fim:=fila_geral_fim+1))))));
  List_Substitution(Implementation(Gerenciador_Senhas_i),cadastrar_senha)==(VAR pos,ii,idx,prev,tmp_val,old_geral IN idx:=ss-1;dono_senha_arr(idx):=vv;estado_senha_arr(idx):=na_espera;bois_corridos_arr(idx):=0;IF torneio_comecou = FALSE THEN pos:=0;IF pos<fila_geral_fim THEN tmp_val:=fila_geral_arr(pos);WHILE pos<fila_geral_fim & tmp_val<ss DO pos:=pos+1;IF pos<fila_geral_fim THEN tmp_val:=fila_geral_arr(pos) END INVARIANT pos: 0..fila_geral_fim & !jj.(jj: 0..fila_geral_fim-1 & jj<pos => fila_geral_arr(jj)<ss) & (pos<fila_geral_fim => tmp_val = fila_geral_arr(pos)) VARIANT fila_geral_fim-pos END END;old_geral:=fila_geral_arr;ii:=fila_geral_fim;WHILE ii>pos DO prev:=ii-1;tmp_val:=fila_geral_arr(prev);fila_geral_arr(ii):=tmp_val;ii:=ii-1 INVARIANT ii: pos..fila_geral_fim & !jj.(jj: 0..fila_geral_fim & jj<=ii => fila_geral_arr(jj) = old_geral(jj)) & !jj.(jj: 0..fila_geral_fim & jj>ii => fila_geral_arr(jj) = old_geral(jj-1)) VARIANT ii-pos END;fila_geral_arr(pos):=ss;fila_geral_fim:=fila_geral_fim+1 ELSE IF ss<ultima_senha THEN fila_geral_arr(fila_geral_fim):=ss;fila_geral_fim:=fila_geral_fim+1 ELSE pos:=0;IF pos<fila_geral_fim THEN tmp_val:=fila_geral_arr(pos);WHILE pos<fila_geral_fim & tmp_val<ss & tmp_val>ultima_senha DO pos:=pos+1;IF pos<fila_geral_fim THEN tmp_val:=fila_geral_arr(pos) END INVARIANT pos: 0..fila_geral_fim & !jj.(jj: 0..fila_geral_fim-1 & jj<pos => fila_geral_arr(jj)<ss & fila_geral_arr(jj)>ultima_senha) & (pos<fila_geral_fim => tmp_val = fila_geral_arr(pos)) VARIANT fila_geral_fim-pos END END;old_geral:=fila_geral_arr;ii:=fila_geral_fim;WHILE ii>pos DO prev:=ii-1;tmp_val:=fila_geral_arr(prev);fila_geral_arr(ii):=tmp_val;ii:=ii-1 INVARIANT ii: pos..fila_geral_fim & !jj.(jj: 0..fila_geral_fim & jj<=ii => fila_geral_arr(jj) = old_geral(jj)) & !jj.(jj: 0..fila_geral_fim & jj>ii => fila_geral_arr(jj) = old_geral(jj-1)) VARIANT ii-pos END;fila_geral_arr(pos):=ss;fila_geral_fim:=fila_geral_fim+1 END END END);
  ...
  List_Substitution(Implementation(Gerenciador_Senhas_i),cancelar_registro_senha)==(VAR pos,ii,idx,prox,tmp_val,lim,old_geral IN idx:=ss-1;dono_senha_arr(idx):=0;estado_senha_arr(idx):=na_espera;bois_corridos_arr(idx):=0;pos:=0;IF pos<fila_geral_fim THEN tmp_val:=fila_geral_arr(pos);WHILE pos<fila_geral_fim & tmp_val/=ss DO pos:=pos+1;IF pos<fila_geral_fim THEN tmp_val:=fila_geral_arr(pos) END INVARIANT pos: 0..fila_geral_fim & !jj.(jj: 0..fila_geral_fim-1 & jj<pos => fila_geral_arr(jj)/=ss) & (pos<fila_geral_fim => tmp_val = fila_geral_arr(pos)) VARIANT fila_geral_fim-pos END END;old_geral:=fila_geral_arr;lim:=fila_geral_fim-1;ii:=pos;WHILE ii<lim DO prox:=ii+1;tmp_val:=fila_geral_arr(prox);fila_geral_arr(ii):=tmp_val;ii:=ii+1 INVARIANT ii: pos..fila_geral_fim-1 & !jj.(jj: pos..fila_geral_fim-1 & jj<ii => fila_geral_arr(jj) = old_geral(jj+1)) & !jj.(jj: pos..fila_geral_fim-1 & jj>=ii => fila_geral_arr(jj) = old_geral(jj)) VARIANT fila_geral_fim-1-ii END;fila_geral_fim:=fila_geral_fim-1 END);
  ...
  List_Substitution(Implementation(Gerenciador_Senhas_i),extrair_lote_rodizio)==(VAR qtd,ii,offset,tmp_val,lim,old_geral IN IF fila_geral_fim>TAMANHO_RODIZIO THEN qtd:=TAMANHO_RODIZIO ELSE qtd:=fila_geral_fim END;old_geral:=fila_geral_arr;ii:=0;WHILE ii<qtd DO tmp_val:=fila_geral_arr(ii);lote_saida_arr(ii):=tmp_val;ii:=ii+1 INVARIANT ii: 0..qtd & !jj.(jj: 0..qtd-1 & jj<ii => lote_saida_arr(jj) = old_geral(jj)) VARIANT qtd-ii END;lote_saida_size:=qtd;lim:=fila_geral_fim-qtd;ii:=0;WHILE ii<lim DO offset:=ii+qtd;tmp_val:=fila_geral_arr(offset);fila_geral_arr(ii):=tmp_val;ii:=ii+1 INVARIANT ii: 0..fila_geral_fim-qtd & !jj.(jj: 0..fila_geral_fim-qtd-1 & jj<ii => fila_geral_arr(jj) = old_geral(jj+qtd)) & !jj.(jj: 0..fila_geral_fim-qtd-1 & jj>=ii => fila_geral_arr(jj) = old_geral(jj)) VARIANT fila_geral_fim-qtd-ii END;fila_geral_fim:=fila_geral_fim-qtd END);
  List_Substitution(Implementation(Gerenciador_Senhas_i),registrar_julgamento_classificacao)==(VAR idx,bc,pos,ii,prev,tmp_val,nbc,old_proxima IN idx:=ss-1;IF jj = zero_boi THEN estado_senha_arr(idx):=eliminada ELSIF jj = valeu_boi THEN bc:=bois_corridos_arr(idx);nbc:=bc+1;bois_corridos_arr(idx):=nbc;IF nbc = BOIS_PARA_CLASSIFICAR THEN estado_senha_arr(idx):=classificada;pos:=0;IF pos<fila_disputa_proxima_fim THEN tmp_val:=fila_disputa_proxima_arr(pos);WHILE pos<fila_disputa_proxima_fim & tmp_val<ss DO pos:=pos+1;IF pos<fila_disputa_proxima_fim THEN tmp_val:=fila_disputa_proxima_arr(pos) END INVARIANT pos: 0..fila_disputa_proxima_fim & !jj2.(jj2: 0..fila_disputa_proxima_fim-1 & jj2<pos => fila_disputa_proxima_arr(jj2)<ss) & (pos<fila_disputa_proxima_fim => tmp_val = fila_disputa_proxima_arr(pos)) VARIANT fila_disputa_proxima_fim-pos END END;old_proxima:=fila_disputa_proxima_arr;ii:=fila_disputa_proxima_fim;WHILE ii>pos DO prev:=ii-1;tmp_val:=fila_disputa_proxima_arr(prev);fila_disputa_proxima_arr(ii):=tmp_val;ii:=ii-1 INVARIANT ii: pos..fila_disputa_proxima_fim & !jj2.(jj2: 0..fila_disputa_proxima_fim & jj2<=ii => fila_disputa_proxima_arr(jj2) = old_proxima(jj2)) & !jj2.(jj2: 0..fila_disputa_proxima_fim & jj2>ii => fila_disputa_proxima_arr(jj2) = old_proxima(jj2-1)) VARIANT ii-pos END;fila_disputa_proxima_arr(pos):=ss;fila_disputa_proxima_fim:=fila_disputa_proxima_fim+1 ELSE estado_senha_arr(idx):=na_espera END ELSE estado_senha_arr(idx):=na_espera END END);
  List_Substitution(Implementation(Gerenciador_Senhas_i),processar_resultado_disputa)==(VAR ss_val,idx,pos,ii,prev,prox,tmp_val,lim,old_atual,old_proxima IN ss_val:=fila_disputa_atual_arr(0);idx:=ss_val-1;old_atual:=fila_disputa_atual_arr;lim:=fila_disputa_atual_fim-1;ii:=0;WHILE ii<lim DO prox:=ii+1;tmp_val:=fila_disputa_atual_arr(prox);fila_disputa_atual_arr(ii):=tmp_val;ii:=ii+1 INVARIANT ii: 0..fila_disputa_atual_fim-1 & !jj2.(jj2: 0..fila_disputa_atual_fim-1 & jj2<ii => fila_disputa_atual_arr(jj2) = old_atual(jj2+1)) & !jj2.(jj2: 0..fila_disputa_atual_fim-1 & jj2>=ii => fila_disputa_atual_arr(jj2) = old_atual(jj2)) VARIANT fila_disputa_atual_fim-1-ii END;fila_disputa_atual_fim:=fila_disputa_atual_fim-1;IF jj = zero_boi THEN estado_senha_arr(idx):=eliminada;eliminados_arr(eliminados_fim):=ss_val;eliminados_fim:=eliminados_fim+1 ELSIF jj = valeu_boi THEN estado_senha_arr(idx):=classificada;pos:=0;IF pos<fila_disputa_proxima_fim THEN tmp_val:=fila_disputa_proxima_arr(pos);WHILE pos<fila_disputa_proxima_fim & tmp_val<ss_val DO pos:=pos+1;IF pos<fila_disputa_proxima_fim THEN tmp_val:=fila_disputa_proxima_arr(pos) END INVARIANT pos: 0..fila_disputa_proxima_fim & !jj2.(jj2: 0..fila_disputa_proxima_fim-1 & jj2<pos => fila_disputa_proxima_arr(jj2)<ss_val) & (pos<fila_disputa_proxima_fim => tmp_val = fila_disputa_proxima_arr(pos)) VARIANT fila_disputa_proxima_fim-pos END END;old_proxima:=fila_disputa_proxima_arr;ii:=fila_disputa_proxima_fim;WHILE ii>pos DO prev:=ii-1;tmp_val:=fila_disputa_proxima_arr(prev);fila_disputa_proxima_arr(ii):=tmp_val;ii:=ii-1 INVARIANT ii: pos..fila_disputa_proxima_fim & !jj2.(jj2: 0..fila_disputa_proxima_fim & jj2<=ii => fila_disputa_proxima_arr(jj2) = old_proxima(jj2)) & !jj2.(jj2: 0..fila_disputa_proxima_fim & jj2>ii => fila_disputa_proxima_arr(jj2) = old_proxima(jj2-1)) VARIANT ii-pos END;fila_disputa_proxima_arr(pos):=ss_val;fila_disputa_proxima_fim:=fila_disputa_proxima_fim+1 ELSE estado_senha_arr(idx):=classificada;fila_disputa_atual_arr(fila_disputa_atual_fim):=ss_val;fila_disputa_atual_fim:=fila_disputa_atual_fim+1 END END);
  List_Substitution(Implementation(Gerenciador_Senhas_i),avancar_rodada_disputa)==(IF fila_disputa_proxima_fim>0 THEN VAR ii,tmp_val IN ii:=0;WHILE ii<fila_disputa_proxima_fim DO tmp_val:=fila_disputa_proxima_arr(ii);fila_disputa_atual_arr(ii):=tmp_val;ii:=ii+1 INVARIANT ii: 0..fila_disputa_proxima_fim & !jj.(jj: 0..fila_disputa_proxima_fim-1 & jj<ii => fila_disputa_atual_arr(jj) = fila_disputa_proxima_arr(jj)) VARIANT fila_disputa_proxima_fim-ii END END;fila_disputa_atual_fim:=fila_disputa_proxima_fim;fila_disputa_proxima_fim:=0;eliminados_fim:=0 ELSE VAR ii,ss_val,idx IN ii:=0;WHILE ii<eliminados_fim DO ss_val:=eliminados_arr(ii);fila_disputa_atual_arr(ii):=ss_val;idx:=ss_val-1;estado_senha_arr(idx):=classificada;ii:=ii+1 INVARIANT ii: 0..eliminados_fim & !jj.(jj: 0..eliminados_fim-1 & jj<ii => fila_disputa_atual_arr(jj) = eliminados_arr(jj)) & !jj.(jj: 0..eliminados_fim-1 & jj<ii => estado_senha_arr(eliminados_arr(jj)-1) = classificada) VARIANT eliminados_fim-ii END END;fila_disputa_atual_fim:=eliminados_fim;eliminados_fim:=0 END);
  List_Substitution(Implementation(Gerenciador_Senhas_i),finalizar_torneio)==(VAR ii,st,old_estado IN old_estado:=estado_senha_arr;ii:=0;WHILE ii<=19 DO st:=estado_senha_arr(ii);IF st = classificada THEN estado_senha_arr(ii):=campea END;ii:=ii+1 INVARIANT ii: 0..20 & !jj.(jj: 0..19 & jj<ii & old_estado(jj) = classificada => estado_senha_arr(jj) = campea) & !jj.(jj: 0..19 & jj<ii & old_estado(jj)/=classificada => estado_senha_arr(jj) = old_estado(jj)) VARIANT 20-ii END END);
...
THEORY VariablesLocEnvX IS
  ...
  Variables_Loc(Implementation(Gerenciador_Senhas_i),cadastrar_senha, 1) == (Type(pos) == Lvl(btype(INTEGER,?,?));Type(ii) == Lvl(btype(INTEGER,?,?));Type(idx) == Lvl(btype(INTEGER,?,?));Type(prev) == Lvl(btype(INTEGER,?,?));Type(tmp_val) == Lvl(btype(INTEGER,?,?));Type(old_geral) == Lvl(SetOf(btype(INTEGER,0,19)*btype(INTEGER,0,MAXINT))));
  Variables_Loc(Implementation(Gerenciador_Senhas_i),atualizar_status, 1) == (Type(idx) == Lvl(btype(INTEGER,?,?)));
  Variables_Loc(Implementation(Gerenciador_Senhas_i),cancelar_registro_senha, 1) == (Type(pos) == Lvl(btype(INTEGER,?,?));Type(ii) == Lvl(btype(INTEGER,?,?));Type(idx) == Lvl(btype(INTEGER,?,?));Type(prox) == Lvl(btype(INTEGER,?,?));Type(tmp_val) == Lvl(btype(INTEGER,?,?));Type(lim) == Lvl(btype(INTEGER,?,?));Type(old_geral) == Lvl(SetOf(btype(INTEGER,0,19)*btype(INTEGER,0,MAXINT))));
  Variables_Loc(Implementation(Gerenciador_Senhas_i),atualizar_dono, 1) == (Type(idx) == Lvl(btype(INTEGER,?,?)));
  Variables_Loc(Implementation(Gerenciador_Senhas_i),extrair_lote_rodizio, 1) == (Type(qtd) == Lvl(btype(INTEGER,?,?));Type(ii) == Lvl(btype(INTEGER,?,?));Type(offset) == Lvl(btype(INTEGER,?,?));Type(tmp_val) == Lvl(btype(INTEGER,?,?));Type(lim) == Lvl(btype(INTEGER,?,?));Type(old_geral) == Lvl(SetOf(btype(INTEGER,0,19)*btype(INTEGER,0,MAXINT))));
  Variables_Loc(Implementation(Gerenciador_Senhas_i),registrar_julgamento_classificacao, 1) == (Type(idx) == Lvl(btype(INTEGER,?,?));Type(bc) == Lvl(btype(INTEGER,?,?));Type(pos) == Lvl(btype(INTEGER,?,?));Type(ii) == Lvl(btype(INTEGER,?,?));Type(prev) == Lvl(btype(INTEGER,?,?));Type(tmp_val) == Lvl(btype(INTEGER,?,?));Type(nbc) == Lvl(btype(INTEGER,?,?));Type(old_proxima) == Lvl(SetOf(btype(INTEGER,0,19)*btype(INTEGER,0,MAXINT))));
  Variables_Loc(Implementation(Gerenciador_Senhas_i),processar_resultado_disputa, 1) == (Type(ss_val) == Lvl(btype(INTEGER,?,?));Type(idx) == Lvl(btype(INTEGER,?,?));Type(pos) == Lvl(btype(INTEGER,?,?));Type(ii) == Lvl(btype(INTEGER,?,?));Type(prev) == Lvl(btype(INTEGER,?,?));Type(prox) == Lvl(btype(INTEGER,?,?));Type(tmp_val) == Lvl(btype(INTEGER,?,?));Type(lim) == Lvl(btype(INTEGER,?,?));Type(old_atual) == Lvl(SetOf(btype(INTEGER,0,19)*btype(INTEGER,0,MAXINT)));Type(old_proxima) == Lvl(SetOf(btype(INTEGER,0,19)*btype(INTEGER,0,MAXINT))));
  ...
  Variables_Loc(Implementation(Gerenciador_Senhas_i),finalizar_torneio, 1) == (Type(ii) == Lvl(btype(INTEGER,?,?));Type(st) == Lvl(etype(STATUS_SENHA,?,?));Type(old_estado) == Lvl(SetOf(btype(INTEGER,0,19)*etype(STATUS_SENHA,0,4))));
